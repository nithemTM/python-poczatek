from a4_4_Słownik_oraz_dict_comprehensions.shop.order import Order, ExpressOrder
from a4_4_Słownik_oraz_dict_comprehensions.shop.data_generator import generate_order
from a4_4_Słownik_oraz_dict_comprehensions.shop.discount_policy import DiscountPolicy, PercentageDiscount, AbsoluteDiscount


def call_object():
    # Zad 1
    order_elements = generate_order(7)
    order_elements_dict = {product_d.product_info.identifier: product_d.product_info for product_d in order_elements}
    print(order_elements_dict)

    # Zad 2 - metoda 1
    order_elements_dict_2 = {key+1: product for key, product in order_elements_dict.items()}
    print(f'Słownik - metoda 1 \n {order_elements_dict_2}')
    # Zad 2 - metoda 2
    order_elements_dict_2 = {product_d.product_info.identifier+1: product_d.product_info for product_d in order_elements}
    print(f'Słownik - metoda 2 \n {order_elements_dict_2}')

    order_elements_dict_2.update(order_elements_dict)
    print(f'Słownik połączony metodą UPDATE \n {order_elements_dict_2}')
